...
export default function MobilityCategories() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const { data } = await api.get('/admin/mobility/categories'); setRows(data.data); }
  ...
  async function save() {
    ...
    setBusy(true);
    try {
      if (mode === 'create') await api.post('/admin/mobility/categories', form);
      else await api.put(`/admin/mobility/categories/${id}`, form);
  ...
  async function remove() {
    setBusy(true);
    try { await api.delete(`/admin/mobility/categories/${del.id}`); toast.success('Category deleted'); setDel(null); load(); }
    catch (e) { toast.error(apiError(e)); } finally { setBusy(false); }
  }
  ...
  return (
    <div>
      <button onClick={() => navigate('/mobility')} className="text-sm text-slate-500 hover:text-slate-700 mb-3">← Back to mobility aids</button>
      <PageHeader title="Mobility Categories" subtitle={`${rows.length} categor${rows.length === 1 ? 'y' : 'ies'}`} actionLabel="+ Add category" onAction={() => setModal({ mode: 'create', form: { ...blank } })} />
      ...
      {modal && (
        <FormModal open title={modal.mode === 'create' ? 'Add category' : 'Edit category'} onClose={() => setModal(null)} onSubmit={save} submitting={busy}>
          {modal.mode === 'create' && (
            <Field label="Code" hint="Lowercase key (e.g. wheelchair). Cannot change later." required>
