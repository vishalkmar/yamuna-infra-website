...
export default function FoodCategories() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const { data } = await api.get('/admin/food/categories'); setRows(data.data); }
  ...
  async function save() {
    ...
    setBusy(true);
    try {
      if (mode === 'create') await api.post('/admin/food/categories', form);
      else await api.put(`/admin/food/categories/${id}`, form);
  ...
  async function remove() {
    setBusy(true);
    try { await api.delete(`/admin/food/categories/${del.id}`); toast.success('Category deleted'); setDel(null); load(); }
    catch (e) { toast.error(apiError(e)); } finally { setBusy(false); }
  }
  ...
  return (
    <div>
      <PageHeader title="Food Ordering" subtitle="Categories → items → orders" actionLabel="+ Add category" onAction={() => setModal({ mode: 'create', form: { ...blank } })}>
        <Link to="/food/orders" className="px-3 py-2 rounded-lg border border-slate-300 text-sm hover:bg-slate-50">Orders</Link>
        <Link to="/food/tiffin" className="px-3 py-2 rounded-lg border border-slate-300 text-sm hover:bg-slate-50">Tiffin plans</Link>
      </PageHeader>

      <DataTable columns={columns} rows={rows} loading={loading} empty="No categories yet"
        actions={r => (
          <div className="flex justify-end gap-3">
            <button onClick={() => navigate(`/food/categories/${r.id}/items`)} className="text-brand-primary font-semibold hover:underline">Items</button>
            <button onClick={() => setModal({ mode: 'edit', id: r.id, form: { name: r.name, icon: r.icon || '', imageUrl: r.imageUrl || '', isActive: !!r.isActive, sortOrder: r.sortOrder } })} className="text-slate-600 hover:underline">Edit</button>
      ...
      {modal && (
        <FormModal open title={modal.mode === 'create' ? 'Add category' : 'Edit category'} onClose={() => setModal(null)} onSubmit={save} submitting={busy}>
          {modal.mode === 'create' && (
            <Field label="Code" hint="Lowercase key (e.g. breakfast). Cannot change later." required>
      ...
      <ConfirmDialog open={!!del} title="Delete category?" message={del ? `“${del.name}” will be removed. Categories with items can’t be deleted.` : ''} confirmLabel="Delete" danger busy={busy} onCancel={() => setDel(null)} onConfirm={remove} />
    </div>
  );
}
