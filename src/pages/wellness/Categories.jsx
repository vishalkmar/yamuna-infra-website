...
export default function WellnessCategories() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const { data } = await api.get('/admin/wellness/categories'); setRows(data.data); }
  ...
  async function save() {
    ...
    setBusy(true);
    try {
      if (mode === 'create') await api.post('/admin/wellness/categories', form);
      else await api.put(`/admin/wellness/categories/${id}`, form);
  ...
  async function remove() {
    setBusy(true);
    try { await api.delete(`/admin/wellness/categories/${del.id}`); toast.success('Category deleted'); setDel(null); load(); }
    catch (e) { toast.error(apiError(e)); } finally { setBusy(false); }
  }
  ...
  return (
    <div>
      <PageHeader title="Wellness & Spiritual" subtitle="Categories → activities" actionLabel="+ Add category" onAction={() => setModal({ mode: 'create', form: { ...blank } })}>
      ...
      <DataTable columns={columns} rows={rows} loading={loading} empty="No categories yet"
        actions={r => (
          <div className="flex justify-end gap-3">
            <button onClick={() => navigate(`/wellness/categories/${r.id}/activities`)} className="text-brand-primary font-semibold hover:underline">Activities</button>
      ...
      {modal && (
        <FormModal open title={modal.mode === 'create' ? 'Add category' : 'Edit category'} onClose={() => setModal(null)} onSubmit={save} submitting={busy}>
          {modal.mode === 'create' && (
            <Field label="Code" hint="Lowercase key (e.g. yoga). Cannot change later." required>
              <input className={inputClass} value={modal.form.code} onChange={e => setF('code', e.target.value.toLowerCase().replace(/\s+/g, '_'))} />
            </Field>
          )}
          <Field label="Name" required><input className={inputClass} value={modal.form.name} onChange={e => setF('name', e.target.value)} placeholder="e.g. Yoga" /></Field>
      ...
      <ConfirmDialog open={!!del} title="Delete category?" message={del ? `“${del.name}” will be removed. Categories with activities can’t be deleted.` : ''} confirmLabel="Delete" danger busy={busy} onCancel={() => setDel(null)} onConfirm={remove} />
    </div>
  );
}
