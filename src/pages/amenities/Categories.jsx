...
export default function AmenityCategories() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const { data } = await api.get('/admin/amenities/categories'); setRows(data.data); }
  ...
  async function save() {
    ...
    setBusy(true);
    try {
      if (mode === 'create') await api.post('/admin/amenities/categories', form);
      else await api.put(`/admin/amenities/categories/${id}`, form);
  ...
  async function remove() {
    setBusy(true);
    try { await api.delete(`/admin/amenities/categories/${del.id}`); toast.success('Category deleted'); setDel(null); load(); }
    catch (e) { toast.error(apiError(e)); } finally { setBusy(false); }
  }
  ...
  return (
    <div>
      ...
      <PageHeader title="Amenity Categories" subtitle={`${rows.length} categor${rows.length === 1 ? 'y' : 'ies'}`} actionLabel="+ Add category" onAction={() => setModal({ mode: 'create', form: { ...blank } })} />
      ...
      {modal && (
        <FormModal open title={modal.mode === 'create' ? 'Add category' : 'Edit category'} onClose={() => setModal(null)} onSubmit={save} submitting={busy}>
          {modal.mode === 'create' && (
            <Field label="Code" hint="Lowercase key (e.g. halls). Cannot change later." required>
      ...
      <ConfirmDialog open={!!del} title="Delete category?" message={del ? `“${del.name}” will be removed. Categories with facilities can’t be deleted.` : ''} confirmLabel="Delete" danger busy={busy} onCancel={() => setDel(null)} onConfirm={remove} />
    </div>
  );
}
