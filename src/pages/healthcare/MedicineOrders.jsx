...
const STATUSES = ['placed', 'out_for_delivery', 'delivered', 'cancelled'];
const LABEL = { placed: 'Placed', out_for_delivery: 'Out for delivery', delivered: 'Delivered', cancelled: 'Cancelled' };
...
export default function MedicineOrders() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const { data: res } = await api.get('/admin/healthcare/medicine-orders', { params });
      setData(res.data);
  ...
  async function changeStatus(o, newStatus) {
    try {
      await api.put(`/admin/healthcare/medicine-orders/${o.id}/status`, { status: newStatus });
      toast.success(`Order #${o.id} → ${LABEL[newStatus]}`);
      load();
  ...
  const columns = [
    ...
    { key: 'items', header: 'Items', render: r => <span className="text-slate-600">{r.items}</span> },
    { key: 'deliveryNote', header: 'Note', render: r => r.deliveryNote || '—' },
  ...
  return (
    <div>
      ...
      <PageHeader title="Medicine Orders" subtitle={`${data.total} order${data.total === 1 ? '' : 's'}`} />
      ...
      <DataTable columns={columns} rows={data.rows} loading={loading} empty="No medicine orders yet" />
